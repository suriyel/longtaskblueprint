...
| **BDD 用例** | `{{HARNESS_MEMORY_DIR}}/plans/bdd.json` | 行为场景（`features[].scenarios[].id` = `BDD-xxx`，每条带 given/when/then/examples）；本节点据此给每个 task 写 `bdd_ids[]`，作为下游 wd/red/gate 的权威 BDD 指针 |
...
1. **阅读已批准的 SRS 和设计文档**
   ...
   - BDD 用例：`{{HARNESS_MEMORY_DIR}}/plans/bdd.json` — 行为场景（`BDD-xxx`），第 4d 步据此给每个 task 填 `bdd_ids[]`
...
4. **从 SRS FR 列表交互式生成功能** — FR 已在需求阶段调整大小（G1-G6 过大 + S1-S4 过小启发式）。粒度已在 SRS 阶段最终确定；此步仅将 FR 映射为功能（1 个 FR 或多个相关 FR → 1 个功能）。
   ...
   d. **按批准结果填充 `items[]`**：
      ...
      - `bdd_ids`：Read `{{HARNESS_MEMORY_DIR}}/plans/bdd.json`，对本功能 `srs_trace` 里的每个 FR，收集所有 `feature.fr` 含该 FR 的 `feature` 下**全部** `scenarios[].id`，去重写入 `bdd_ids[]`。**这是 wd / red / gate_red / gate_behavior 的权威 BDD 指针——下游不再重新做 `fr ∩ srs_trace` 推导。** 纯内部、bdd.json 无对应场景的 task 可为空数组。
   ...
   e. **验证门禁**（inline 校验）：填充所有功能后人工检查：
      ...
      - **bdd.json 中每个 `scenarios[].id` 至少出现在一个 task 的 `bdd_ids[]` 中（无孤立 BDD 场景）；每个 `bdd_ids[]` 项在 bdd.json 中真实存在** —— 下游 `gate_init` 硬门机检这一条，缺即打回。
...
[
  {
    ...
    "bdd_ids": ["BDD-001","BDD-007"], // L3 optional: array; 取自 bdd.json 中 feature.fr ∩ 本 task.srs_trace 的 feature 下全部 scenario.id（权威 BDD 指针）
...
3. 调脚本（`--lang` 命中 preset 时自动填工具默认值，显式 flag 覆盖）：
   ...
   ```
4. 产出：`{{HARNESS_MEMORY_DIR}}/plans/project-context.md`（下游 wd / red / green / refactor 读取的**权威源**：tech_stack 与项目级 constraints / assumptions 均以此为准；task 对象不再单独承载约束/假设）
