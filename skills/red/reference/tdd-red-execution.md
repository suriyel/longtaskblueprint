...
2. **§接口契约** -- 方法签名、前/后置条件、边界决策表、错误处理表。当注释为 "Uses: [§11.1 library]" 时，测试设置可对该 §11.1 库 mock/stub，而非被替代的方案。
   - **绝对例外 —— BDD 可观察面禁 mock**：若某模块/函数正是产出某 BDD 场景 `then` 子句可观察结果的那张表面（即测试清单"追踪到"含 `BDD-xxx` 的行所断言的最终可观察输出 —— 随技术栈而定，如返回值、输出/渲染文本、状态码、持久化记录、发出的事件等），则**禁止 mock/stub 替换它**：必须以真实实现产出该可观察值并断言其**精确值**。mock 只允许出现在比该可观察面更外层的真实系统边界（外部网络调用/三方服务/时钟/文件系统等）。下游 `gate_behavior` 硬门会真跑测试并核对：可观察面被整模块 mock、或只断言"被调用"而未断言 `then` 精确值 → 打回本节点。
