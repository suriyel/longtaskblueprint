...
下游 `gate_red` 用来判覆盖的那把尺子，就是一段**纯静态、不跑测试、几秒出结果、与语言无关**的现成脚本。本节点收尾、触发推进之前，先自己跑一遍把窟窿堵上，别等门禁打回再补：

1. 运行 `node {{SCRIPTS}}/gate_red.cjs`，读末行 JSON / 文本报告（**只读它的 stdout**，无需自己打开 `bdd.json`，不违反上面「规格输入」的禁令）。
2. 报告若列出「某些相关 `BDD-xxx` 场景未在测试代码中找到 id 标记」→ **当场为每个缺失场景补一条带 id 标记的失败测试**（标记规则同上：紧邻注释写 `# BDD-001`，不进方法名），再跑，**直到报告通过**才收尾。
3. 这是**自纠不是自证**：`gate_red` 节点收尾后仍会独立再跑同一脚本（双跑兜底），所以自检的目标是让那道门第一次就过，而非省略它。
4. **护栏**：若反复自检后确认某相关场景的 `then` 精确预期值在 wd §测试清单中根本不存在（= wd 漏列/漏写，red 无从下手）→ 走上面的「降级护栏」返回 BLOCKED 指向 wd，**不臆造测试、不硬编预期值**糊过自检。
