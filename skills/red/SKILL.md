...
- 为「"追踪到"含 `BDD-xxx`」的**每一行**写测试时，**必须用紧邻注释/注解给测试打标**（`# BDD-001` / `// BDD-001`，紧贴该测试上方），使「BDD 场景 → 测试代码」追溯链在代码层可见。**追溯编号只进注释，不得写入方法名**（不要 `..._BDD001` / `..._bdd4` 这类后缀）。
- **UT 命名（三段式 given/when/then）**：测试方法名按 `given_<前置>_when_<触发>_then_<期望>` 三段组织，复述该行场景的前置/触发/期望；随技术栈调整大小写与分隔（如 Java `givenXxx_whenYyy_thenZzz`、JS `test('given… when… then…')`）。名字只描述行为，不含编号或排序号；负向用例在 `then` 段写明拒绝/失败/异常（如 `then_reject` / `then_raise_*` / `then_return_4xx`）。
- 覆盖每个带 `BDD-xxx` 的清单行，**一个都不能少**：下游 `gate_red` 硬门 grep 工作区测试文件逐 id 核对（注释里的编号即可命中），缺任一即打回本节点。
...
- **BDD 追溯打标**：§测试清单"追踪到"含 `BDD-xxx` 的行，对应测试必须带该 id 注释标记（不进方法名）——gate_red 据此判定，优先级高于"精简"
- 先写集成测试，再写单元测试
