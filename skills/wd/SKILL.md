...
## 交活前自检（pre-flight，收尾推进前必做）

本节点是整条线里**反馈链最长**的一步：§测试清单漏场景要等 `gate_red` 才暴露，「预期」写虚要再隔 red→green→refactor 三步才在 `gate_behavior` 被打回。所以交设计文档前，先照这两条自查、当场修，把问题堵在源头：

1. **清单全不全**：逐条核对 `{{TASK_GET}}` 的 `task.bdd_ids` —— 每个 `BDD-xxx` 在 §测试清单里**至少有一行**，且该行「追踪到」列带其 id。缺任一 → 补行。（这正是下游 `gate_red` 要查的，提前到设计阶段堵掉，下游 red 才有完整清单可写。）
2. **预期实不实**：逐行检查 BDD 派生行的「预期」列，是否都落到了**具体可断言的值**（精确字符串/状态码/返回结构/持久化记录等，随技术栈而定），有没有「显示成功」「返回正确」「调用成功」这类**无法精确断言的虚词**。发现虚词 → 回到该行，用 §接口契约 的边界/Raises 把它改写成精确值，并确认该行已标注**可观察面**（产出此 `then` 的真实模块/符号，供下游 red 判断哪张面禁 mock）。
3. 这是**自纠不是自证**：下游 `gate_red` / `gate_behavior` 仍会独立核验，自检只为让它们第一次就过、而非取代它们。
4. **护栏**：若自查发现 `task.bdd_ids` 为空但本特性显然有行为面、或某场景 `then` 的精确值在 `bdd.json`/SRS 里根本取不到 → 按上面规则走 BLOCKED 指向上游（init / 需求），**不臆造预期值**糊过自检。
