...
> 本功能与其它功能点、存量逻辑的交互/协作面，以及应复用、扩展或作为模式参考的既有代码（含本次迭代已生成的）。最大化复用 -- 优先绑定真实实现而非编写新代码或 mock。

**协作特性与集成接缝**（来自 §1c-0 接触面识别 + §1c-1 真实实现定位）：

> 列出本功能调用、被调用或共享状态的每个特性/模块。`状态`：passing=已实现、pending=待实现、external=非本 loop 的存量逻辑。被消费的接口须给出真实实现 `file:line`；Provider 待实现时记 §6.2 契约 ID 并标 pending。**不得用 mock 顶替真实可用或计划中的接口。**

| # | 协作特性/模块 | 关系 | 状态 | 真实接口或 §6.2 契约 | 绑定决策 |
|---|---------------|------|------|---------------------|----------|
| 1 | [特性 #ID / 模块名] | [消费 / 被调 / 共享] | [passing / pending / external] | [真实符号或端点 @ file:line，或 §6.2 契约 ID] | [REUSE 真实实现，不 mock / 绑定契约，集成测试 pending] |

[无协作方时写 "本功能无跨功能协作 -- 自包含。"]

**现有代码复用**（通过代码库探索和已通过依赖功能发现，本功能应复用、扩展或作为模式参考）：
...
- [ ] 协作特性与集成接缝表完整 -- 每个协作方标 passing/pending/external；passing 协作方与被消费的 §6.2 接口已给真实实现 file:line，或显式标 pending（无 mock 顶替真实可用接口）
