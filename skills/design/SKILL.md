...
动作：合成单一方案（用户指定则校核，未指定则直接推荐），经方案校核表确认偏差 → 内部装配整份草稿 → 落盘 `{{HARNESS_MEMORY_DIR}}/plans/design.md`（**状态**: 待审批）→ Step 5c 单次批准门批准。

<HARD-GATE>
在收到 Step 5c 单次批准门的"批准"回复之前：
- 不可 `git commit` 设计文档（仅允许写入 `**状态**: 待审批` 草稿）
- 不可把状态置「已批准」、不可删除文件末尾 `## 待审阅事项`
- 不可编写任何代码、搭建任何项目、采取任何实现行动
- 整份一次性呈现并单次批准，不按章节分段征求逐节反馈；自检 / 一致性检查通过 ≠ 用户已批准，禁止自行标记已批准并提交
...
落盘后进入 Step 5b 集成一致性检查（追加 `## 待审阅事项`）；整体呈现与单次批准统一在 Step 5c 完成。
...
## Step 5c — 整体呈现、单次批准、收尾

完成 Step 5b 集成一致性检查后：

1. 自检上述 6 项检查清单，确认每项已 PASS 或合理 PASS-SKIPPED（LLM 自检层，**不替代用户审批**）。
2. **整体呈现待审批**：输出文本告知用户已落盘整份设计文档的路径 + 需阅读全文（含 §0-§11 + 文件末尾 `## 待审阅事项`）+ 阅后回对话做一次性整体批准。**不分章节呈现、不逐节征求反馈。**
3. **单次批准门**：通过**一次** `AskUserQuestion` 征求整份批准：`[批准] / [要修改（说明改哪里）]`。
   - **批准** → 编辑 `{{HARNESS_MEMORY_DIR}}/plans/design.md`：首行 `**状态**: 待审批` → `**状态**: 已批准`；删除文件末尾 `## 待审阅事项` 整段；`git commit`；照常推进。
   - **要修改** → 按用户说明修订**整份**草稿后重新落盘（回 Step 5）+ 重跑 Step 5b + 重新呈现 + 重新征求一次性批准（仍是整体单次，不退化为逐章）。
4. **收到「批准」前不可把状态置「已批准」、不可删除待审阅事项、不可 git commit、不可推进到 init 阶段。**
